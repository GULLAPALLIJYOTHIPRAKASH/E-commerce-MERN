...
function AddProductsSidebar({SubmitEditProduct , editStatus , previewImg, HandleImgUpload , HandleAddProducts  , HandlePreviewImg , ImageRef , addProductsform ,HandleAllInputs ,  showAddProducts,HandleShowAddProducts}){
    ...
    <aside className={`products-sidebar-container w-[100%] h-[100%] fixed flex justify-end top-0 ${showAddProducts ? "left-0" : "left-[100%]"} bg-black/30 transition-all linear duration-500` }>
        <div className="products-sidebar-center w-[70vw]  overflow-hidden overflow-y-auto    sm:w-[50vw] lg:w-[30vw] bg-white p-6">
            <div className="heading w-[100%] flex justify-between items-center">
                <h1 className="text-lg font-medium">{editStatus ? "Edit Product" : "Add New Product"}</h1>
            ...
            <div className="form-section mt-5">
                <form className="products-form">
                    ...
                    <div className="field mb-4">
                        <label  className="text-base font-normal ">Upload a Image</label>
                        <input onChange={HandleImgUpload} ref={ImageRef}  type="file" name="productimg" id="productimg" accept="/*" className="hidden" />

                        { !addProductsform?.image ? 
                    ...
                    <div className="btn">
                       {editStatus  ? <button onClick={SubmitEditProduct} className="w-[100%] p-2 bg-black text-white outline-none rounded-lg text-base cursor-pointer hover:opacity-80 transition-all linear duration-300">Edit</button>:
                        <button onClick={HandleAddProducts} className="w-[100%] p-2 bg-black text-white outline-none rounded-lg text-base cursor-pointer hover:opacity-80 transition-all linear duration-300">Add</button>}
                    </div>
                </form>
            </div>
        </div>
